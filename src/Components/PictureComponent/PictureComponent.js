import React, { Component } from 'react'
import PropTypes from 'prop-types'
import { Card, Avatar } from 'antd'
import { Popover, Button } from 'antd';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import moment from "moment";
import firebase from "../../firebase"
// import { faThumbsUp, faThumbsDown, faHeart, faComment, faPaperPlane } from '@fortawesome/free-regular-svg-icons'
import { UserOutlined, DeleteTwoTone } from '@ant-design/icons';
import { AiOutlineHeart, AiFillHeart } from 'react-icons/ai';
import { FiMessageCircle } from 'react-icons/fi';
import { RiShareForwardBoxLine } from 'react-icons/ri';
import { BsDot } from 'react-icons/bs';
import "./PictureComponent.css"
import axios from 'axios';
import blurbg from "../../images/blurbg.jpg"
var storageRef = firebase.storage().ref();
export class PictureComponent extends Component {
    constructor(props){
        super(props);
        this.state = {
            title: "First Post",
            content: "This is the content of first post.",
            // imgsrc : storageRef.child('posts/'+props.post.postId+".jpg").getDownloadURL().then((url) =>{
            //     let img = document.getElementById(props.post.postId);
            //     img.src = url;
            //     return url
            // }),
            imgsrc:"",
            createdBy : "",
            userName:"",
            showLiked:false,
            avatar:"data:image/jpeg;base64,"
        }
        // this.getUrl=()=>{
        //     let upload = storageRef.child('posts/'+props.post.postId+".jpg").getDownloadURL().then((url) =>{return url});
        //     console.log(upload);
        // }
        // console.log(storageRef.child('posts/'+props.post.postId+".jpg").getDownloadURL().then((url) =>{return url}));
    }

    componentWillReceiveProps(props){
        if(props !== this.props){
            this.setState({
                createdBy:`${props.User.firstName} ${props.User.lastName}`,
                userName:props.User.userName
            })
        }
    }
    
    deletemypost=()=>{
        console.log("i m here")
        let id = this.props.post.postId;
        let formdata = {
            "postId":this.props.post.postId,
            "likeCount":this.props.User.userId
        }
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/post/deletePost`,formdata).then((res)=>{
            console.log(res);
            if(res.data===true){
                console.log("delete successfull");
                let index = this.props.postMap.get(id);
                this.props.deleteMyPost(index);
                this.props.setMyPosts("Pankaj is god");
            }
            else{
                console.log("delete unsuccessfull");  
            }
        })
    }

    likeThisPost = () =>{
        let post = this.props.post;
        let likedBy = post.likedUsers;
        likedBy[likedBy.length] = this.props.User.userId;
        post.likedUsers = likedBy;
        post.postName = "";
        let formdata = {
            "post":post,
            "mobile":this.props.User.mobileNumber
        };
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/post/addNewPost`,formdata).then((res)=>{
            console.log(res.data);
            this.setState({
                showLiked:true
            })
        })
    }

    unlikeThisPost = () =>{
        let post = this.props.post;
        let likedBy = post.likedUsers;
        // likedBy[likedBy.length] = this.props.User;
        let index = likedBy.indexOf(this.props.User.userId);
        if (index > -1) {
            likedBy.splice(index, 1);
          }
        post.likedUsers = likedBy;
        let formdata = {
            "post":post,
            "mobile":this.props.User.mobileNumber
        };
        axios.post(`${process.env.REACT_APP_BACKEND_URL}/post/addNewPost`,formdata).then((res)=>{
            console.log(res.data);
            this.setState({
                showLiked:false
            })
        })
    }


    render() {
        const agotime = moment(this.props.post.createDate).fromNow();
        const  createdBy= !this.props.fromProfile ? `${this.props.post.firstName} ${this.props.post.lastName}`:`${this.props.User.firstName} ${this.props.User.lastName}`;
        
        return (
            <div className={this.props.fromProfile ? "picture-component-full-profile" : "picture-component-full"}>
                {this.props.fromProfile && !this.props.fromHeader && <DeleteTwoTone onClick={()=>this.deletemypost()} className="deletemypost" />}
                <Card  bordered={true} style={{textAlign:"left"}}>

                    <div>
                        
                        {/* <img src = {this.state.avatar} style={{display:'inline',height:'18px' , width: '18px' , borderRadius:'50%'}}/> */}
                        <Avatar className="post-avatar"  size={20} icon={<UserOutlined />} />
                    <p style={{display:'inline' , fontSize:'15px' , textAlign:'center'}}>{createdBy}</p>
                    <span className="agoTime">{agotime}</span>
                    </div>
                    {/* <hr /> */}
{this.props.fromProfile &&                <div className="qwerty" style={{backgroundImage:`url(${blurbg})`}}>    <img className="qwertyimg" src = {this.props.post.imgUrl} style={{marginTop:"10px"}}/></div>
}                   
{!this.props.fromProfile &&             <div className="qwerty2" style={{backgroundImage:`url(${blurbg})`}}>    <img   className="qwertyimg" src = {this.props.post.imgUrl} style={{marginTop:"10px", maxHeight:"200px",maxWidth:"600px"}}/></div>
} 
                    {/* <br /> */}
                    <div className="post-tools">
                        <div className="left-tools">
                            {(!this.props.post.likedUsers.includes(this.props.User.userId)) && <AiOutlineHeart onClick={()=>{this.likeThisPost()}} size={30} style={{marginRight:"10px", cursor:"pointer"}}/>}
                            {(this.props.post.likedUsers.includes(this.props.User.userId)) && <AiFillHeart onClick={()=>{this.unlikeThisPost()}} size={30} style={{marginRight:"10px",color:"red", cursor:"pointer"}}></AiFillHeart>}
                            <Popover title="Developer is coding me right now.">
                            <FiMessageCircle onClick={()=>{}} size={30} style={{marginRight:"10px", cursor:"pointer"}}/>
                            </Popover>
                            <Popover title="Developer is coding me right now.">
                            <RiShareForwardBoxLine onClick={()=>{}} size={30} style={{marginRight:"10px", cursor:"pointer"}}/>
                            </Popover>
                        </div>
                        <div className="right-tools">
                           <p>{this.props.post.likedUsers.length} Likes  </p>
                        </div>
                    </div>

                    {/* <button href="#" style={{border:'none' , backgroundColor:'white' , padding:'0px' , margin:'0px 15pxpx 0px 0px'}} ><FontAwesomeIcon icon={faHeart} /></button>
                    <button href="#" style={{border:'none' , backgroundColor:'white' , padding:'0px' , margin:'0px 0px 0px 15px'}} ><FontAwesomeIcon icon={faComment} /></button>
                    <button href="#" style={{border:'none' , backgroundColor:'white' , padding:'0px' , margin:'0px 0px 0px 15px'}} ><FontAwesomeIcon icon={faPaperPlane} /></button> */}
                    <div>
            {this.props.post.postDescription.length > 180 && <Popover placement="bottom"  content={this.props.post.postDescription} >
                <p className="post-desc" style={{}}><span style={{fontWeight:"bold", marginRight:"10px"}}>{!this.props.fromProfile?this.props.post.userName:this.props.post.userName}</span>{this.props.post.postDescription.length > 180 ? this.props.post.postDescription.substr(0,180) + "...":this.props.post.postDescription}</p>
               </Popover>}
               {this.props.post.postDescription.length <= 180 &&  
                <p className="post-desc" style={{}}><span style={{fontWeight:"bold", marginRight:"10px"}}>{!this.props.fromProfile?this.props.post.userName:this.props.post.userName}</span>{this.props.post.postDescription.length > 180 ? this.props.post.postDescription.substr(0,180) + "...":this.props.post.postDescription}</p>
               }
                    </div>

                </Card>
            </div>
        )
    }
}

export default PictureComponent
